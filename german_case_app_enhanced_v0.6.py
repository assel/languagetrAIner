# German Case Master v0.6 - Comprehensive Learning App
# Major improvements from v0.1 through v0.6:
# - v0.1: Original prototype with basic diagnostic + analysis
# - v0.2: Enhanced explanations + better feedback  
# - v0.3: Added practice modes + hints system
# - v0.4: Comprehensive features (spaced repetition, achievements, multiple modes)
# - v0.5: Major bug fixes (10 critical bugs resolved)
# - v0.6: Mixed practice bugs fixed + form validation bug resolved
...
def get_adaptive_questions(target_case=None, difficulty='adaptive', num_questions=5):
    ...
    # BUG FIX: Handle mixed practice correctly
    if target_case:
        # Single case practice - get questions for that case only
        available_questions = PRACTICE_QUESTIONS.get(target_case, []).copy()
        quiz_questions_filtered = [q for q in QUIZ_QUESTIONS if q['case'] == target_case]
    else:
        # Mixed practice - combine ALL practice questions from ALL cases
        for case_name, questions in PRACTICE_QUESTIONS.items():
            available_questions.extend(questions.copy())
        quiz_questions_filtered = QUIZ_QUESTIONS.copy()  # Include all quiz questions
    
    # Convert quiz questions to practice format and add them
    for q in quiz_questions_filtered:
        available_questions.append({
            ...
            'id': q['id'],
            'case': q['case']  # BUG FIX: Add case info for hint system
        })
    
    if not available_questions:  # Fallback if no questions
        available_questions = [{'question': 'No questions available', 'correct': 'N/A', 'context': 'general', 'case': 'unknown'}]
...
if st.session_state.current_mode == 'case_selection':
    ...
    if st.session_state.practice_questions:
        ...
        # BUG FIX: Better progress calculation and question numbering
        if st.session_state.practice_questions_original_count > 0:
            current_question_number = st.session_state.practice_questions_original_count - len(st.session_state.practice_questions) + 1
            progress = (current_question_number - 1) / st.session_state.practice_questions_original_count
        else:
            current_question_number = 1
            progress = 0
        
        st.progress(progress)
        st.write(f"**Question {current_question_number} of {st.session_state.practice_questions_original_count}:** {current_q['question']}")
        
        # Context and case info display
        info_parts = []
        if 'context' in current_q:
            context_emoji = {
                'family': '👨‍👩‍👧‍👦', 'education': '📚', 'food': '🍎', 'weather': '🌤️',
                'transportation': '🚌', 'shopping': '🛒', 'entertainment': '🎭',
                'general': '💬', 'everyday_description': '🏠', 'everyday_action': '🚶',
                'family_relationships': '👥', 'social_relationships': '🤝', 'time_seasons': '📅'
            }
            context_display = f"{context_emoji.get(current_q['context'], '💬')} {current_q['context'].replace('_', ' ').title()}"
            info_parts.append(f"**Context:** {context_display}")
        
        if 'case' in current_q:
            case_emoji = {'nominativ': '🔵', 'akkusativ': '🟢', 'dativ': '🟡', 'genitiv': '🔴'}
            case_display = f"{case_emoji.get(current_q['case'], '⚪')} {current_q['case'].title()}"
            info_parts.append(f"**Case:** {case_display}")
        ...
        with col2:
            if st.button("💡 Hint", use_container_width=True):
                # BUG FIX: Improved hint system using case info from question
                hints = {
                    "dativ": "Ask yourself: 'To whom?' or 'For whom?' Look for indirect objects and dativ prepositions.",
                    "akkusativ": "Ask yourself: 'Whom?' or 'What?' This is usually the direct object of an action verb.",
                    "genitiv": "Ask yourself: 'Whose?' This case shows possession or follows certain prepositions.",
                    "nominativ": "Ask yourself: 'Who?' or 'What?' This is the subject doing the action."
                }
                
                # Use case info from question if available
                if 'case' in current_q and current_q['case'] in hints:
                    case_hint = hints[current_q['case']]
                else:
                    # Fallback: try to detect from question content
                    case_hint = "Think about the grammar rule for this case!"
                    question_lower = current_q.get('question', '').lower()
                    if any(word in question_lower for word in ['gebe', 'helfe', 'gehört', 'mit', 'bei', 'von', 'zu']):
                        case_hint = hints['dativ']
                    elif any(word in question_lower for word in ['sehe', 'kaufe', 'lese', 'durch', 'für', 'ohne']):
                        case_hint = hints['akkusativ']
                    elif any(word in question_lower for word in ['trotz', 'während', 'wegen']):
                        case_hint = hints['genitiv']
                    elif any(word in question_lower for word in ['ist', 'sind', 'war', 'spielt']):
                        case_hint = hints['nominativ']
    ...
    with st.form("comprehensive_diagnostic"):
        ...
        # BUG FIX: Store which questions are actually shown to user
        questions_shown = []
        ...
        for i, q in enumerate(questions_to_use):
            ...
            # Track which questions we actually showed
            questions_shown.append(q['id'])
            st.divider()
        
        submitted = st.form_submit_button("🚀 Analyze My German Skills!", use_container_width=True)
        
        if submitted:
            # BUG FIX: Only validate questions that were actually shown to user
            unanswered_ids = []
            for q_id in questions_shown:
                if not answers.get(f"q_{q_id}", "").strip():
                    unanswered_ids.append(q_id)
            
            if unanswered_ids:
                # Show question numbers based on position in the form
                unanswered_positions = []
                for q_id in unanswered_ids:
                    position = questions_shown.index(q_id) + 1
                    unanswered_positions.append(position)
                
                st.error(f"Please answer all questions! Missing: {', '.join(map(str, unanswered_positions))}")
            else:
...
col1, col2, col3 = st.columns(3)
with col1:
    st.caption("🇩🇪 German Case Master v0.6")
with col2:
