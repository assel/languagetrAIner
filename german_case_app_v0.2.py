...
def get_detailed_explanation(question_id, user_answer, correct_answer, is_correct=False):
    """Generate detailed explanation for each specific question - both correct and incorrect answers"""
    
    # Get the full sentence for context
    question_data = next(q for q in QUIZ_QUESTIONS if q['id'] == question_id)
    full_sentence = question_data['question'].replace('_______', f'**{correct_answer}**')
    ...
    explanations = {
        1: {
            ...
            'why_right': 'You correctly identified this as the subject of the sentence',
            'memory_tip': 'Ask: WHO is very nice? → Der Mann (subject = Nominativ)',
            'pattern': 'Masculine Nominativ: der Mann (unchanged from dictionary form)'
        },
        2: {
        ...
        4: {
            ...
            'why_right': 'You correctly recognized that "geben" needs Dativ (to whom?)',
            'memory_tip': 'Ask: I give TO WHOM? → der Frau (feminine Dativ: die → der)',
            'pattern': 'Feminine Dativ: die Frau → der Frau'
        },
        5: {
            ...
            'why_right': 'You correctly used possessive pronoun + noun in Dativ case',
            'memory_tip': 'sein + Dativ masculine = seinem, Bruder stays Bruder',
            'pattern': 'Possessive Dativ masculine: sein Bruder → seinem Bruder'
        },
        6: {
            'context': 'After preposition "mit" (Dativ)',
            'rule': '"mit" always takes Dativ case',
            'why_wrong': 'All transportation with "mit" uses Dativ case',
            'why_right': 'You correctly remembered that "mit" always takes Dativ',
            'memory_tip': 'mit + masculine Dativ: der Bus → dem Bus',
            'pattern': 'Masculine Dativ after preposition: der Bus → dem Bus'
        },
        7: {
            ...
            'why_right': 'You correctly used possessive pronoun in Genitiv to show possession',
            'memory_tip': 'Whose car? → meiner Schwester (feminine Genitiv: meine → meiner)',
            'pattern': 'Possessive Genitiv feminine: meine Schwester → meiner Schwester'
        },
        8: {
            ...
            'why_right': 'You correctly used Genitiv after "trotz" with proper adjective ending',
            'memory_tip': 'trotz + neuter Genitiv: das → des, adjective gets -en ending',
            'pattern': 'Neuter Genitiv with adjective: das schlechte Wetter → des schlechten Wetters'
        },
        9: {
            ...
            'why_right': 'You correctly recognized that students are indirect objects (explained TO)',
            'memory_tip': 'Plural Dativ: die → den, adjective gets -en ending',
            'pattern': 'Plural Dativ with adjective: die neuen Studenten → den neuen Studenten'
        },
        10: {
            'context': 'After preposition "mit" (Dativ)',
            'rule': '"mit" takes Dativ, possessive + adjective must match',
            'why_wrong': 'Need possessive pronoun + adjective in Dativ masculine',
            'why_right': 'You correctly declined both possessive and adjective in Dativ masculine',
            'memory_tip': 'mit + sein + Dativ masc. = seinem alten (both get -em/-en)',
            'pattern': 'Possessive + adjective Dativ masculine: sein alter Freund → seinem alten Freund'
        },
        11: {
            ...
            'why_right': 'You correctly used Genitiv after "während" with proper adjective ending',
            'memory_tip': 'während + masculine Genitiv: der → des, adjective gets -en',
            'pattern': 'Masculine Genitiv with adjective: der lange Winter → des langen Winters'
        },
        12: {
            'context': 'Showing possession (Genitiv)',
            'rule': 'Possession: "die Mutter des kleinen Mädchens" = the mother OF the little girl',
            'why_wrong': 'Need neuter Genitiv with adjective declension', 
            'why_right': 'You correctly used Genitiv to show possession with proper adjective ending',
            'memory_tip': 'Whose mother? → des kleinen Mädchens (neuter Genitiv: das → des)',
            'pattern': 'Neuter Genitiv with adjective: das kleine Mädchen → des kleinen Mädchens'
        }
    }
    ...
    if question_id in explanations:
        ...
        # Show the complete sentence first
        sentence_display = f"**Complete sentence:** {full_sentence}\n\n"
        
        if is_correct:
            return f"""✅ **Perfect!** "{correct_answer}" is correct.

{sentence_display}💡 **Why this works:** {exp['rule']}  
📝 **Context:** {exp['context']}  
✅ **What you did right:** {exp['why_right']}  
🎯 **Pattern:** {exp['pattern']}
"""
        else:
...
{sentence_display}💡 **Why:** {exp['rule']}  
📝 **Context:** {exp['context']}  
❗ **Your mistake:** {exp['why_wrong']}  
🎯 **Pattern to remember:** {exp['pattern']}
"""
    else:
    ...
    for q in QUIZ_QUESTIONS:
        ...
        if user_answer.lower() == correct.lower():
            ...
            analysis["detailed_feedback"].append({
                "question": q["id"],
                "correct": True,
                "explanation": get_detailed_explanation(q["id"], user_answer, correct, is_correct=True)
            })
        else:
            ...
            analysis["detailed_feedback"].append({
                "question": q["id"], 
                "correct": False,
                "explanation": get_detailed_explanation(q["id"], user_answer, correct, is_correct=False)
            })
